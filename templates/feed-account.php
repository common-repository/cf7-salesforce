...
.vxcf_options_row .howto{
  ...
  <?php
    ...
    foreach($msgs as $msg){
     if(isset($msg['class']) && $msg['class'] !=""){
          ?>
  <div class="fade below-h2 <?php echo esc_attr($msg['class']) ?> notice is-dismissible">
  ...
  ?>
  <div class="vx_wrap"> 
    <h2 class="vx_img_head"><img alt="<?php esc_html_e("Salesforce Feeds", 'cf7-salesforce') ?>" title="<?php esc_html_e("Salesforce Feeds", 'cf7-salesforce') ?>" src="<?php echo esc_url($this->get_base_url())?>images/salesforce-crm-logo.png?ver=1" /> <?php 
echo sprintf(esc_html__('Edit Salesforce Feed # %d','cf7-salesforce') ,$fid); 
  if(empty($id) || $new_account_id != $id){
 ?> <a href="<?php echo esc_url($new_feed_link) ?>" class="add-new-h2"><?php esc_html_e("Add New Feed",'cf7-salesforce'); ?></a> 
 <?php
}
 ?>
 <a href="<?php echo esc_url($feeds_link) ?>" class="add-new-h2"><?php esc_html_e("Back to Feeds",'cf7-salesforce'); ?></a>
...
if(empty($feed_name)){
  ...
  ?>
  <div class="error" id="message" style="margin-top:20px;"><?php echo wpautop(sprintf(esc_html__("We are unable to login to Salesforce with the provided API token. Please make sure they are valid in the %sSettings Page%s", 'cf7-salesforce'), "<a href='".esc_url($this->link_to_settings())."'>", "</a>")); ?></div>
  <?php
  // return;
  } 
  ?>
  ...
  <?php wp_nonce_field("vx_nonce") ?>
    <div class="vx_div">
      <div class="vx_head">
<div class="crm_head_div"> <?php esc_html_e('1. Enter feed name',  'cf7-salesforce' ); ?></div>
...
<div class="crm_clear"></div> 
  ...
  <div class="vx_row">
  <div class="vx_col1">
  <label for="vx_name" class="left_header"><?php esc_html_e("Feed Name", 'cf7-salesforce'); ?>
  ...
  <div class="vx_div">
        <div class="vx_head">
<div class="crm_head_div"> <?php esc_html_e('2. Select Salesforce Account.', 'cf7-salesforce'); ?></div>
<div class="crm_btn_div" title="<?php esc_html_e('Expand / Collapse','cf7-salesforce') ?>"><i class="fa crm_toggle_btn vx_action_btn fa-minus"></i></div>
<div class="crm_clear"></div> 
  </div>
  ...
  <div class="vx_group">
    <div class="vx_row">
  <div class="vx_col1">
  <label for="vx_account" class="left_header"><?php esc_html_e("Salesforce Account", 'cf7-salesforce'); ?>
  <?php $this->tooltip("vx_sel_object") ?>
 </label>
  </div>
  <div class="vx_col2">
  <select id="vx_account" class="crm_sel load_object" name="account" autocomplete="off">
  <option value=""><?php esc_html_e("Select a Salesforce Account", 'cf7-salesforce'); ?></option>
  <?php
  ...
  <div class="clear"></div>
    ...
    <div class="vx_div">
        <div class="vx_head">
<div class="crm_head_div"> <?php esc_html_e('3. Select Contact Form.', 'cf7-salesforce'); ?></div>
<div class="crm_btn_div" title="<?php esc_html_e('Expand / Collapse','cf7-salesforce') ?>"><i class="fa crm_toggle_btn vx_action_btn fa-minus"></i></div>
<div class="crm_clear"></div> 
  </div>
  ...
  <div class="vx_group">
    <div class="vx_row">
  <div class="vx_col1">
  <label for="vx_form_id" class="left_header"><?php esc_html_e("Contact Form Form", 'cf7-salesforce'); ?>
  <?php $this->tooltip("vx_sel_form") ?>
 </label>
  </div>
  <div class="vx_col2">
  <select id="vx_form_id" class="crm_sel load_object" name="form_id" autocomplete="off">
  <option value=""><?php esc_html_e("Select a Form", 'cf7-salesforce'); ?></option>
  <?php
...
<?php
  ...
  ?>  
<div class="howto"><?php echo sprintf(esc_html__('If contact form is missing from list then install our %s , submit required form, it will add that missing form in this list.','cf7-salesforce'),'<a href="https://wordpress.org/plugins/contact-form-entries/" target="_blank">Contact Form Entries</a>')?></div>
<?php
      }
?>
  </div>
  <div class="clear"></div>
    ...
    <div id="crm_ajax_div_object" style="display:none; text-align: center; line-height: 100px;"><i> <?php esc_html_e('Loading, Please Wait...','cf7-salesforce'); ?></i></div>
...
$(document).on("change",".vxc_field_option",function(e){
  ...
  $.post(ajaxurl,{action:'refresh_data_<?php echo esc_html($this->id) ?>',vx_crm_ajax:vx_crm_ajax,vx_action:action,account:account},function(res){
...
//post validation
 ...
 if($(".vx_required").length){
   ...
   var value=""; 
    ...
    attach_error.append('<div class="entry_row vx_entry_error"><div class="vx_error"><i class="fa fa-warning"></i> <?php esc_html_e('This is a required field','cf7-salesforce') ?></div></div>');
    return false;
    }  
   })  
 }
  ...
  e.preventDefault(); 
    if(!confirm('<?php esc_html_e('Are you sure to remove ?','cf7-salesforce') ?>')){
      return;
  }
  ...
  if(field_name_select.length){
    ...
    if(field_name == '' || crm_fields[field_name] == ''){
     alert('<?php esc_html_e('Please Select Field Name','cf7-salesforce') ?>');
     return;   
    }  
...
if(type){ row.find('.vxc_field_option').hide(); }
  ...
  $(document).on('click','.vxc_tips',function(e){
    ...
    jQuery('#crm_note_fields').select2({ placeholder: '<?php esc_html_e('Select Field','cf7-salesforce') ?>'});
} 
...
vx_select();
  function vx_select(){
      $('#vx_add_fields_select').select2({ placeholder: '<?php esc_html_e('Select Field','cf7-salesforce') ?>'});
	  add_note_sel();
  }
