...
class vxcf_sales {
  ...
  public static $title='Contact Form Salesforce Plugin';  
  public static $path = ''; 
  public static $slug = "";
         ...
         $base_url=get_site_url();   
          ...
          foreach($val as  $vv){
              ...
              $vv= wp_get_attachment_url($vv) ;
               if(strpos(ABSPATH,'/wp-content/') === false){ //elementor hosting adds /wp-content/ to ABSPATH and does not allow file access , continue with url
                $vv=str_replace($base_url,trim(ABSPATH,'/'),$vv); 
              }     
              $files[]=$vv;
          } }
...
self::$plugin->instance();
  ...
  */
  ...
  if($type == "admin"){
     if($ver<4.2){
  ?>
    <div class="error vx_notice notice" data-id="<?php echo esc_attr($id) ?>"><p style="display: table"><span style="display: table-cell; width: 98%"><span class="dashicons dashicons-megaphone"></span> <b><?php esc_html_e('Contact Form Salesforce Plugin','cf7-salesforce') ?>. </b><?php echo wp_kses_post($message);?> </span>
<span style="display: table-cell; padding-left: 10px; vertical-align: middle;"><a href="#" class="notice-dismiss" title="<?php esc_html_e('Dismiss Notice','cf7-salesforce') ?>">dismiss</a></span> </p></div>
  <?php
     }else{
  ?>
  <div class="error vx_notice notice below-h2 is-dismissible" data-id="<?php echo esc_attr($id) ?>"><p><span class="dashicons dashicons-megaphone"></span> <b><?php esc_html_e('Contact Form Salesforce Plugin','cf7-salesforce') ?>. </b> <?php echo wp_kses_post($message);?> </p>
  </div>    
  <?php
     }
  }else{
  ...
  */
  ...
  echo '<div class="'. esc_attr( $level ) .' notice below-h2 is-dismissible"><p>';
...
*/
  ...
  $message = sprintf(__("%sContact Form Entries%s is required. %s it today!%s", 'cf7-salesforce'), "<a href='https://www.crmperks.com/'>", "</a>", "<a href='https://www.crmperks.com/'>", "</a>");   
  }else if($installed === 2){ // not active
  $message = sprintf(__('Contact Form Entries is installed but not active. %sActivate Contact Form Entries%s to use the Contact Form Salesforce Plugin','cf7-salesforce'), '<strong><a href="'.wp_nonce_url(admin_url('plugins.php?action=activate&plugin=contact-form-entries-pro/contact-form-entries-pro.php'), 'activate-plugin_contact-form-entries-pro/contact-form-entries-pro.php').'">', '</a></strong>');  
  } else if($installed === 3){ // not supported
  $message = sprintf(__("A higher version of %sContact Form Entries%s is required. %sPurchase it today!%s", 'cf7-salesforce'), "<a href='https://www.crmperks.com/'>", "</a>", "<a href='https://www.crmperks.com/'>", "</a>");
  }  
...
$value=trim(implode(',',$value));
  ...
  */
  ...
  $link="N/A"; $desc=__("Added to ",'cf7-salesforce');
    $status_imgs=array("1"=>"created","2"=>"updated","4"=>"filtered",'5'=>'deleted');
    
     if(!empty($feed['object']) && isset($objects[$feed['object']])){
      $feed['object']=$objects[$feed['object']];
  }
  if( !empty($feed['object']) && !empty($feed['status'])){
  ...
  $desc.=$feed['object'];
  if($feed['status'] == 5){
   $desc=sprintf(__("%s Deleted",'cf7-salesforce'),$feed['object']);   
  }if($feed['status'] == 4){
      $desc=sprintf(__("%s Filtered",'cf7-salesforce'),$feed['object']); 
  }
  }else{
  $desc= !empty($feed['error']) ? $feed['error'] : esc_html__("Unknown Error",'cf7-salesforce');
  }
   if($feed['status'] == 2 && empty($feed['crm_id']) && !empty($feed['error'])){
      $desc=$feed['error'];  
    }
  $title=__("Failed",'cf7-salesforce');   
  if($feed['status'] == 1){
  $title=__("Created",'cf7-salesforce');   
  }else if($feed['status'] == 2){
  $title=__("Updated",'cf7-salesforce');   
  }else if($feed['status'] == 4){
  $title=__("Filtered",'cf7-salesforce');   
  }else if($feed['status'] == 5){
  $title=__("Deleted",'cf7-salesforce');   
  }
  ...
  */
  ...
  printf('<input type="checkbox" name="'.esc_attr($this->id).'_update" id="'.esc_attr($this->id).'_update" value="1" /><label for="'.esc_attr($this->id).'_update" title="%s">%s</label><br /><br />', esc_html__('Create or update this entry in Salesforce. The fields will be mapped according to the form feed settings.', 'cf7-salesforce'), esc_html__('Send to Salesforce', 'cf7-salesforce'));
  } else {
...
if(!current_user_can($this->id."_send_to_crm")){return; }
  ...
  $button.= '<input type="submit" class="button button-large button-secondary alignright" name="'.esc_attr($this->id).'_send" style="margin-left:5px; '.$margin.'" title="'.__('Create or update this entry in Salesforce. The fields will be mapped according to the form feed settings.','cf7-salesforce').'" value="'.__('Send to Salesforce', 'cf7-salesforce').'" onclick="jQuery(\'#action\').val(\'send_to_crm\')" />';
  //logs button
    ...
    $button.= '<a class="button button-large button-secondary alignright" style="margin-left:5px; margin-top:5px; " title="'.__('Go to Salesforce Logs','cf7-salesforce').'" href="'.esc_url($log_url).'">'.__('Salesforce Logs','cf7-salesforce').'</a>';
...
return  $var;
  ...
  */
  ...
  $status=0;
  if(class_exists('vxcf_sales_wp')){
      $status=1;
  }else if( file_exists(WP_PLUGIN_DIR.'/cf7-salesforce/cf7-salesforce.php')) {
  $status=2;
  } 
  return $status;
  }
...
$entry['_vx_updated']=isset($entry['__vx_entry']['updated']) ? $entry['__vx_entry']['updated'] : $local_time;  
 ...
 if(!empty($entry['__vx_entry'])){  
  $entry['_vx_url']=$entry['__vx_entry']['url'];     
     ...
     $entry['_vx_url']=$this->post('HTTP_REFERER',$_SERVER);
 }
$this->form_fields='';
...
$api_type=$date_format='';
if(isset($info['data']['api']) ){
$api_type=$info['data']['api'];
$date_format=get_option( 'date_format' );
if( $info['data']['api'] =="web"){
   if(!empty($entry['_vx_updated'])){
     $entry['_vx_updated']=date('m/d/Y h:i A',strtotime($entry['_vx_updated']));
 }
if(!empty($entry['_vx_created'])){
$entry['_vx_created']=date('m/d/Y h:i A',strtotime($entry['_vx_created'])); //removed @16feb-24 api does not underatnd 01/10/2024
 } 
...
$value=$this->process_tags($entry,$value); 
  ...
  if($field !=""){
  $value=$this->verify_field_val($entry,$field,$k); 
  if($value == ''){ $value=null;  }
  if(isset($this->form_fields[$field]) && in_array($this->form_fields[$field]['type'],array('checkbox','acceptance')) && isset($this->form_fields[$field]['values']) && count($this->form_fields[$field]['values'])< 2 ){
 if(is_array($value)){ $value=trim(implode(' ',$value)); }
  ...
  if($api_type == 'web'){
      if(isset($this->form_fields[$field]) && in_array($this->form_fields[$field]['type'],array('date'))){
        if(!empty($this->form_fields[$field]['values']) && !empty($this->form_fields[$field]['values'][0]['value'])){
       $date_format=$this->form_fields[$field]['values'][0]['value'];
  ...
  if(isset($custom[$k]) && !is_null($value)){
 if(isset($custom[$k]['name_c'])){
 $k=$custom[$k]['name_c'];   $custom[$k]['label']=$k;  
 }
...
//var_dump($temp); die(); 
  ...
  if(isset($data['owner']) && !empty($data['user']) ){
   $temp['OwnerId']=array('value'=>apply_filters('vx_assigned_user_id',$data['user'],$this->id,$feed['id'],$entry) ,'label'=>'Owner ID');   
  }
...
if(!empty($data['contact_check']) && !empty($data['contact_feed'])){ 
  ...
  if(!empty($contact_res['id'])){
   $temp['related_to']=array( $contact_res['id']);   
  }else{ //if empty continue
      continue;
  }  
   }
...
$temp=apply_filters('crmperks_salesforce_post',$temp,$feed,$entry);
  ...
  if( $added){
      ...
      if($action == "Added"){
          if(empty($res['link'])){
  $msg=sprintf(__('Successfully Added to Salesforce (%s) with ID # %s .', 'cf7-salesforce'),$feed['object'],$res['id']);
          }else{
  $msg=sprintf(__('Successfully Added to Salesforce (%s) with ID # %s . View entry at %s', 'cf7-salesforce'),$feed['object'],$res['id'],$res['link']);
          }
  $screen_msg=__( 'Entry added in Salesforce', 'cf7-salesforce');
      }else{
            if(empty($res['link'])){
  $msg=sprintf(__('Successfully Updated to Salesforce (%s) with ID # %s . View entry at %s', 'cf7-salesforce'),$feed['object'],$res['id'],$res['link']);   
            }else{
  $msg=sprintf(__('Successfully Updated to Salesforce (%s) with ID # %s .', 'cf7-salesforce'),$feed['object'],$res['id']);   
            }
          if($event == 'delete'){  
     $screen_msg=__( 'Entry deleted from Salesforce', 'cf7-salesforce');
          }else{
     $screen_msg=__( 'Entry updated in Salesforce', 'cf7-salesforce');
          }
          }
   
  
  }else if($this->post('status',$res) == 4){
  $screen_msg=$msg=__( 'Entry filtered', 'cf7-salesforce');    
  }else if($this->post('status',$res) == 6){ 
      $status='2';   $screen_msg=$error=$res['error'];
  }else{
  $status=0; $screen_msg_class="error";
  $screen_msg=__('Errors when adding to Salesforce. Entry not sent! Check the Entry Notes below for more details.' , 'cf7-salesforce' );
  if($log_id!=""){
      //message for  bulk actions in logs
  $screen_msg=__('Errors when adding to Salesforce. Entry not sent' , 'cf7-salesforce' );    
  }
  $msg=sprintf(__('Error while creating %s', 'cf7-salesforce'),$feed['object']);
...
$this->send_error_email($info,$entry,$form);
  ...
  $log_link=' <a href="'.esc_url($log_url).'" class="vx_log_link" data-id="'.$insert_id.'">'.__('View Detail','cf7-salesforce')."</a>";
 $screen_msg.=$log_link;
    }
    if($post_comment){
...
//$this->add_note($entry["id"], $msg);
  ...
  */
  ...
  $detail["Form Id"]=$form['id'];
    ...
    $email_info=array("msg"=>$info['msg'],"title"=>__('Salesforce','cf7-salesforce')." Error","info_title"=>"More Detail","info"=>$detail);
  ...
  */
  ...
  if($api_type == 'web'){
  return array("Lead"=>"Lead","Case"=>"Case");
  }
   $objects=array();      
      if(!empty($info)){   
  ...
  */
  ...
  if(vxcf_sales::$is_pr === false && in_array($v['name'],$phone)){ continue; }
  $fields[$v['name']]=$v;    
  }
  foreach($map as $k=>$v){
  if(isset($v['name_c']))
...
$vxcf_sales=new vxcf_sales();
$vxcf_sales->instance();
$vx_cf['vxcf_sales']='vxcf_sales';
endif;
