...
if( !class_exists( 'vxcf_sales_pages' ) ) {
...
* @since       1.0.0
*/
class vxcf_sales_pages   extends vxcf_sales{
    public $ajax=false;
...
public function add_bulk_send_crm_callcenter($list){ 
 $list['send_salesforce']=__('Send to Salesforce','cf7-salesforce');
  return $list;  
}
public function bulk_send_crm_callcenter($msg,$action,$ids,$type){ 
    if($action == 'send_salesforce'){
...
$entry['__vx_id']=$entry['id'];
        ...
        if(empty($notice)){
            $notice='Nothing Sent to Salesforce';
        }
...
$push=$this->push($entry_detail,$form,'add_note');
  ...
  ?>
  <div class="vx_row">
  <label><input type="checkbox" name="<?php echo esc_attr($this->id) ?>_send" value="yes"> <?php esc_html_e('Send to Salesforce','cf7-salesforce') ?></label>
  </div>
  <?php  
}
    /**
...
$lead['__vx_id']=$id;
   ...
   $slug=$this->get_slug();
      if ( $file == $slug ) {
          $settings_link=$this->link_to_settings();
            array_unshift( $links, '<a href="' .$settings_link. '">' . esc_html__('Settings', 'cf7-salesforce') . '</a>' );
        }
        return $links;
   }
  /**
  ...
  */
  ...
  // Adding submenu if user has access
        $page_title =__('Salesforce for Contact Form','cf7-salesforce');
        $menu_title =__('Salesforce','cf7-salesforce');
...
$class='';
 if($res){
       $msg=__('Account Deleted Successfully','cf7-salesforce');
  $class='updated';   
 }else{
       $msg=__('Error While Removing Account','cf7-salesforce');
  $class='error';      
 }
...
die();
  ...
  check_admin_referer('vx_nonce','vx_nonce');
  if(!current_user_can($this->id."_edit_settings")){ 
  $msg=__('You do not have permissions to add token','cf7-salesforce');
...
$info_data=$api->handle_code();
 ...
 /* if(!empty($token)){
  ...
  $msg=__('You do not have permissions to export logs','cf7-salesforce');
  $this->display_msg('admin',$msg);
  return;   
  }
  ...
  $fields=array(); $field_titles=array("#",__('Status','cf7-salesforce'),__('Salesforce ID','cf7-salesforce') ,__('Entry ID','cf7-salesforce'),__('Description','cf7-salesforce'),__('Time','cf7-salesforce'));
  ...
  check_admin_referer('vx_nonce','vx_nonce');
  if(!current_user_can($this->id."_edit_settings")){ 
  $msg=__('You do not have permissions to clear logs','cf7-salesforce');
  $this->display_msg('admin',$msg);
  return;   
  }
  $data=$this->get_data_object();
  $clear=$data->clear_logs();
       ...
       $msg=__('Error While Clearing Salesforce Logs','cf7-salesforce');
      $level="error";
      if(!empty($clear)){
      $msg=__('Salesforce Logs Cleared Successfully','cf7-salesforce');   
      $level="updated";
      }
  ...
  //
  self::$tooltips = array(
 'vx_feed_name' =>  esc_html__('Enter feed name of your choice.', 'cf7-salesforce'),
  'vx_sel_object' => esc_html__('Select the Object to Create when a Form is Submitted.', 'cf7-salesforce'),
   'vx_sel_account' =>__('Select the Salesforce account you would like to export entries to.', 'cf7-salesforce'),
  'vx_sel_form' => esc_html__('Select the Contact Form you would like to integrate with Salesforce. Contacts generated by this form will be automatically added to your Salesforce account.', 'cf7-salesforce'),
  
  'vx_map_fields' => esc_html__('Associate your Salesforce fields to the appropriate Contact Form fields.', 'cf7-salesforce'),
  
  'vx_optin_condition' =>__('When the opt-in condition is enabled, form submissions will only be exported to Salesforce when the condition is met. When disabled all form submissions will be exported.', 'cf7-salesforce'),
  
  'vx_manual_export' => esc_html__('If you do not want all entries sent to Salesforce, but only specific, approved entries, check this box. To manually send an entry to Salesforce, go to Entries, choose the entry you would like to send to Salesforce, and then click the "Send to Salesforce" button.', 'cf7-salesforce'),
  
    'vx_entry_notes' => esc_html__('Enable this option if you want to synchronize Contact Form entry notes to Salesforce Object notes. For example , when you add a note to a Contact Form entry, it will be added to the Salesforce Object selected in the feed.', 'cf7-salesforce'),
    
      'vx_primary_key' => esc_html__('Which field should be used to update existing objects?', 'cf7-salesforce'),
      
  'vx_web' => sprintf(__('Web-to-Lead is available for all Salesforce Editions. If you are not sure if your Salesforce Edition supports the API, you should use Web-to-Lead. Editions that do not support the Salesforce API: %s 1: Personal Edition %s 2: Group Edition %s 3: Professional Edition %s Note: You can purchase API access for a Professional Edition', 'cf7-salesforce'),'<br/>','<br/>','<br/>','<br/>'),
  'vx_api' => sprintf(__('The API features are more powerful than Web-to-Lead. You can create different object types, as well as other advanced features. If you have any of the following Salesforce Editions, you can use the included API Add-on: %s 1: Enterprise Edition %s 2: Unlimited Edition %s 3: Developer Edition %s 4: Professional Edition - Requires API Upgrade', 'cf7-salesforce'),'<br/>','<br/>','<br/>','<br/>','<br/>'),
  
  'vx_custom_app'=>__('This option is for advanced users who want to override default Salesforce App.','cf7-salesforce'),
  
  'vx_disable_logs'=>__('When an order is sent to Salesforce we store that order information in the database and show it in the Salesforce Log. Check this box if you do not want to save the exported order information in the logs.','cf7-salesforce'),
  
  'vx_line_items'=>__('Create a Salesforce Order product for each Woocommrce Cart item.','cf7-salesforce'),
  
  'vx_price_books'=>__('Get PriceBooks list from salesforce.','cf7-salesforce'),
  'vx_camps'=>__('Get Campaigns and Status list from salesforce.','cf7-salesforce'),
  
  'vx_sel_price_book'=>__('Which Pricebook should be searched for product','cf7-salesforce'),
  'vx_sel_camp'=>__('Which Campaign should be assigned to this object.','cf7-salesforce'),
  'vx_sel_status'=>__('What should be Member Status.','cf7-salesforce'),
  
  'vx_pro_desc'=>__('A new product will be created in selected Pricebook. You can add a description for new products created by this plugin.','cf7-salesforce'),
  
   'vx_assign_account'=>__('Enable this option if you want to assign an account this object.','cf7-salesforce'),
   'vx_sel_account'=>__('Object created by this feed will be assigned to the selected Account.','cf7-salesforce'),
   
      'vx_assign_contract'=>__('Enable this option , if you want to assign a Contract to this object','cf7-salesforce'),
   'vx_sel_contract'=>__('Select Contract feed. Contract created by this feed will be assigned to this object','cf7-salesforce'),
   
   'vx_camp_check'=>__('If enabled, Lead/Contact will be added to selected Campaign','cf7-salesforce'),
   'vx_owner_check'=>__('Enable this option if you want to assign another object owner.','cf7-salesforce'),
   'vx_owners'=>__('Get Users list from Salesforce','cf7-salesforce'),
   'vx_order_notes'=>__('Enable this option if you want to synchronize WooCommerce Order notes to Salesforce Object notes. For example, when you add a note to a WooCommerce Order, it will be added to the Salesforce Object selected in the feed.','cf7-salesforce'),
   'vx_sel_owner'=>__('Select a user as a owner of this object','cf7-salesforce'),
   
         'vx_entry_note'=>__('Check this option if you want to send more data as CRM entry note.', 'cf7-salesforce'),
   'vx_note_fields'=>__('Select fields which you want to send as a note', 'cf7-salesforce'),
   'vx_disable_note'=>__('Enable this option if you want to add note only for new CRM entry', 'cf7-salesforce'),
...
<div class="table_head_i"><?php esc_html_e('Capsule', 'cf7-salesforce'); ?></div>  
<div class="vx_group">
...
echo $this->format_log_msg($log);
 ...
 $msg=__('You do not have permissions for this action','cf7-salesforce');
...
if( $this->has_feed($form_id)) { 
 $boxes['salesforce_crm']=array('title'=>'<i class="fa fa-plug"></i> '.__('Salesforce', 'cf7-salesforce'),'callback'=>array($this,'meta_box_html'));  
} return $boxes; 
}
...
public function has_feed($form_id) {
 ...
 if(!empty($info['meta']) ){
  ...
  switch($action){
    ...
    $res['status']="ok";
    $data['crm_sel_camp']=$this->gen_select($camps,$status_sel,__('Select Campaign','cf7-salesforce'));
    ...
    if(is_array($camps)){
    $res['status']="ok";
    $data['crm_sel_status']=$this->gen_select($status_list,$camp_id_sel,__('Select Status','cf7-salesforce'));
  ...
  case"refresh_users":
    ...
    if(is_array($users)){
    $res['status']="ok";
    $data['crm_sel_user']=$this->gen_select($users,$owner_sel,__('Select User','cf7-salesforce'));
    $meta['users']=$users;   
    }else{
     $res['error']=$users;   
    }
...
if(isset($res['error'])){
    $res['status']='error';
    if(empty($res['error'])){
    $res['error']=__('Unknown Error','cf7-salesforce');
    }
}
  die(json_encode($res));    
  }
  ...
  */
  public  function mapping_page(){
      ...
      wp_enqueue_style('vxc-tooltip');
         $tabs=array('feeds'=>__('Salesforce Feeds','cf7-salesforce') , 'logs'=>__('Salesforce Log','cf7-salesforce') , 'accounts'=>__('Salesforce Accounts','cf7-salesforce'), 'settings'=>__('Settings','cf7-salesforce'));
  ...
  $(this).text('<?php esc_html_e('Show Key','cf7-salesforce') ?>');  
  $(this).removeClass('vx_hidden');
  key.attr('type','password');  
  }else{
  $(this).text('<?php esc_html_e('Hide Key','cf7-salesforce') ?>');  
...
start_tooltip();  
  ...
  */
  ...
  <p>
  <?php esc_html_e("Feed deleted.", 'cf7-salesforce') ?>
  </p>
  </div>
  <?php
  }
  ...
  <div class="updated fade" style="margin:10px 0;">
  <p>
  <?php esc_html_e("Feeds deleted.", 'cf7-salesforce') ?>
  </p>
  </div>
  <?php
  }
...
$count=$this->data->delete_log($log_ids);
  $this->screen_msg(sprintf(__('Successfully Deleted %d Item(s)','cf7-salesforce'),$count));  
    }
  ...
  if(!empty($entry['__vx_data'])){ 
    ...
    $push=array('class'=>'error','msg'=>__('Entry Not Found','cf7-salesforce'));  
  }
  ...
  case"time": $time_order=$order_icon; $time_class="";   break;    
    ...
    $bulk_actions=array(""=>__('Bulk Action','cf7-salesforce'),"delete"=>__('Delete','cf7-salesforce'),
  'send_to_crm_bulk'=>__('Send to Salesforce','cf7-salesforce'),'send_to_crm_bulk_force'=>__('Force Send to Salesforce - Ignore filters','cf7-salesforce'));
  $base_url=$this->get_base_url();
   $objects=get_option($this->id.'_meta',array()); 

   $objects=$this->post('objects',$objects);
      $statuses=array("1"=>__("Created",'cf7-salesforce'),"2"=>__("Updated",'cf7-salesforce'),"error"=>__("Failed",'cf7-salesforce'),"4"=>__("Filtered",'cf7-salesforce'),"5"=>__("Deleted",'cf7-salesforce')); 
...
//if($module == 'Opportunity'){ $has_contact=true; }
//
}else{
    $map_fields=$fields;
}

  $sel_fields=array(""=>__("Standard Field",'cf7-salesforce'),"value"=>__("Custom Value",'cf7-salesforce'));
  ...
  */
  ...
  public function set_logging_supported($plugins) {
      $slug=$this->plugin_dir_name(); 
        $plugins[$slug] = esc_html__('Salesforce','cf7-salesforce');
        return $plugins;
    }
  /**
...
$this->ajax=true;
  ...
  $msg=__("Please Choose Object",'cf7-salesforce');
  }else  if(empty($_REQUEST['form_id'])){
  $msg=__("Please Choose Form",'cf7-salesforce');
  }
  if($msg !=""){
  ?>
...
/*  $object=$this->post('object',$feed);
  ...
  $msg=__("Please Choose Account",'cf7-salesforce');
  }else  if(empty($_REQUEST['form_id'])){
  $msg=__("Please Choose Form",'cf7-salesforce');
  }
  if($msg !=""){
...
$field_options="<option>".esc_html__("Select Object",'cf7-salesforce')."</option>"; 
  if(is_array($objects)){
...
die();
  ...
  */
  ...
  if(!current_user_can($this->id.'_read_settings')){
  $msg_text=__('You do not have permissions to access this page','cf7-salesforce');   
  ...
  $this->uninstall();
  $uninstall_msg=sprintf(__("Contact Form Salesforce Plugin has been successfully uninstalled. It can be re-activated from the %s plugins page %s.", 'cf7-salesforce'),"<a href='plugins.php'>","</a>");
$this->screen_msg($uninstall_msg);
  ...
  $meta=isset($_POST['meta']) ? $this->post('meta') : array();
   ...
   $msgs['submit']=array('class'=>'updated','msg'=>__('Settings Changed Successfully','cf7-salesforce'));
...
include_once(self::$path . "templates/settings.php");
 ...
 $offset=$this->time_offset();
  ...
  check_admin_referer("vx_nonce");
  if(!current_user_can($this->id."_edit_settings")){ 
  esc_html_e('You do not have permissions to save settings','cf7-salesforce');
  return;   
  }
  ...
  if(!$valid_email){
      $msgs['submit']=array("class"=>"error","msg"=>__('Invalid Email(s)','cf7-salesforce'));
  }
   $info=$this->get_info($id); 
   $crm=$info['data'];
...
//  $force_check=true;
  ...
  if($con_class != "updated" ){
      $msg=__('Connection to Salesforce is NOT Working','cf7-salesforce');  
  }else{
     $msg=__('Connection to Salesforce is Working','cf7-salesforce');   
  }
  $title=__('Test Connection: ','cf7-salesforce');
...
$msgs=array();
  ...
  check_admin_referer("vx_nonce");
  if(!current_user_can($this->id.'_edit_feeds')){
  esc_html_e('You do not have permissions to edit/save feed','cf7-salesforce'); 
  return;
  }
  //
...
if(is_array($feed_update) && is_array($feed)){
  ...
  if($is_valid){
      ...
      $msgs['save']=array('class'=>'updated','msg'=>sprintf(__("Feed Updated. %sback to list%s", 'cf7-salesforce'), '<a href="'.$feed_link.'">', "</a>"));
  }
  else{
  $msgs['save']=array('class'=>'error','msg'=>__("Feed could not be updated. Please enter all required information below.", 'cf7-salesforce'));
...
if(is_array($tags)){
  ...
  $fields['__vx_id']=array('id'=>'__vx_id','label'=>__('Entry ID','cf7-salesforce')); 
$fields['_vx_form_id']=array('id'=>'_vx_form_id','label'=>__('Form ID','cf7-salesforce')); 
$fields['_vx_form_name']=array('id'=>'_vx_form_title','label'=>__('Form Title','cf7-salesforce')); 
$fields['_vx_url']=array('id'=>'_vx_url','label'=>__('Page URL','cf7-salesforce')); 
$fields['_vx_created']=array('id'=>'_vx_created','label'=>__('Entry Created','cf7-salesforce')); 
$fields['_vx_updated']=array('id'=>'_vx_updated','label'=>__('Entry Updated','cf7-salesforce')); 

  $this->fields=$fields=array('cf'=>array("title"=>__('Contact Form Fields','cf7-salesforce'),"fields"=>$fields));
  ...
  $fields['feeds']=array("title"=>__('ID from other Feeds','cf7-salesforce'),"fields"=>$feeds);
  } }
  $this->fields=$fields;
  }
  return $fields;
...
if( in_array($option_k,$sel_val)){
  ...
  */
  ...
  $modules=array(""=>__("Select Object",'cf7-salesforce'));
...
$api_check=(int)$this->post('valid_api',$info);
  ...
  //////////////////
  if($res === NULL){
  $info['msg']=__('Invalid Org ID. Please check your Org ID and try again.','cf7-salesforce'); 
  $info['class']="error";        
  }else{
  $info['msg']=__('Success: Your Org ID is properly configured','cf7-salesforce'); 
  $info['class']="updated";
  }  
  ...
  $msg=__( 'Successfully Connected to Salesforce','cf7-salesforce' );
     if(isset($info['sales_token_time'])){
       $msg.=" - ".date('F d, Y h:i:s A',$info['sales_token_time']);
   }
      $info['msg']=$msg; 
  $info['class']="updated";     
